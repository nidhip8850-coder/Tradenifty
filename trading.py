...
def compute_signal(df):
    ...
    reasons.append(f"Bullish Engulf: {bullish_engulf}")
...
while True:
    sig = latest_signal.get("signal", "NO DATA")
    reasons = latest_signal.get("reasons", [])
    time_now = latest_signal.get("time", "--:--:--")

    st.markdown(f"### ⏰ Last Updated: **{time_now}**")
